...
const unblockPost = fn<{postId: string; token: string}, void>(
  async (args, _, {req, res}) => {
    ...
    if (verify(args.token)) {
      ...
      blocklist = await removeIdFromBlocklist(args.postId, blocklist)
